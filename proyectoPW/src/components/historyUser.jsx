  ...
  useEffect(() => {
    try {
      ...
      if (userId && token && role && username) {
        setUserData({ userId, token, role, username });
        setLoading(false);
      } else {
      ...
      <div id='User' className="w-1/3">
        <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow">
          <div className="flex flex-col items-center pb-10 py-8">
            ...
            <span className="my-2 text-sm font-medium text-black">{userData.username}</span>
            <span className="my-2 text-sm font-medium text-black">{userData.role}</span>
