...
const RegisterForm = () => {
    ...
    const onSubmitPersonalInfo = async (data) => {
        try {
            // Guardar datos personales en el estado local o en el almacenamiento
    ...
    const onSubmitCredentials = async (data) => {
        ...
        const userData = { ...personalInfo, ...data };

        try {
            // Primero, crea el usuario con los datos personales
            const userResult = await getFetchData("POST", "/user/create", userData);
            if (userResult) {
                // Luego, crea las credenciales
                const credentialData = { username: data.username, password: data.password };
                const credentialResult = await getFetchData("POST", "/credential/create", credentialData);
                ...
                alert("Error en el registro del usuario. Inténtalo nuevamente.");
            }
        } catch (error) {
            console.error("Error al enviar los datos:", error);
            alert("Hubo un problema al intentar registrarte. Inténtalo de nuevo más tarde.");
        }
    };
        ...
        <div className="font-[sans-serif] bg-white md:h-screen">
            <div className="grid md:grid-cols-2 items-center gap-8 h-full">
                <div className="hidden bg-cover lg:block lg:w-2/3">
                    <img src="/register.png" className="object-contain block mx-32" alt="register-image" />
                </div>
                <div className="flex items-center md:p-8 p-6 bg-black h-full lg:w-11/12 lg:ml-auto">
                    <form 
                        onSubmit={handleSubmit(step === 1 ? onSubmitPersonalInfo : onSubmitCredentials)} 
                    ...
                    >
                        {step === 1 ? (
                            <>
                                ...
                                <div className="mt-8">
                                    <button type="submit" className="bg-[#fcf149] text-black py-2 px-4 rounded-lg w-full hover:bg-yellow-300">Siguiente</button>
                                </div>
                            </>
                        ) : (
                            <>
                                <div className="mb-10">
                                    <h3 className="text-3xl font-bold text-[#fcf149]">Crea tus credenciales</h3>
                                </div>

                                <div>
                                    <label htmlFor="username" className="text-white text-xs block mb-2">Nombre de Usuario</label>
                                    <div className="relative flex items-center">
                                        <input
                                            {...register('username')}
                                            type="text"
                                ...
                                <div className="mt-8">
                                    <button type="submit" className="bg-[#fcf149] text-black py-2 px-4 rounded-lg w-full hover:bg-yellow-300">Registrar</button>
                                </div>
                            </>
                        )}
                    </form>
